...
import Module6.Part8.common.GeneralUtils;
import Module6.Part8.common.MyLogger;
...
public class Room implements AutoCloseable {
		...
		try {
			...
			if(message.startsWith("/flip")){
				...
				int coin = (int) ( Math.random() * 2 + 1); // will return either 1 or 2
					...
					if (coin == 1){ sendMessage(client,"#r#<b>"+client.getClientName()+"#r#<b>"+" #b#<b>has flipped a coin and it landed on heads<b>#b#"); }
					else {sendMessage(client,client.getClientName()+" #b#<b>has flipped a coin and it landed on tails<b>#b#");}}

			if(message.startsWith("/roll")){
            
				int dice = (int) ( Math.random() * 6 + 1); // will return either 1 - 6
				
				sendMessage(client,"#r#<b>"+client.getClientName()+"#r#<b>"+" #b#<b>has rolled the dice and it landed on #b#<b>" + dice); }

			if(message.startsWith("@")){
            
				String newMessage = whisperGetMessage(message);
				String targetClientName = whisperGetClientName(message);
				long clientID = getClientIdByName(targetClientName);
				client.sendMessage(clientID, newMessage); }
	...
	//block username
	public String removeExclamationMark(String str) {
		if (str.startsWith("!")) {
			str = str.substring(1);
		}
	...
	//whisper function
	public String whisperGetClientName(String input) {
		String output = input;
	...
	//whisper get message function
	public static String whisperGetMessage(String input) {
		int indexOfFirstSpace = input.indexOf(" ");
		if (indexOfFirstSpace == -1) {
			// There are no space characters in the input string
			return input;
		} else {
	...
	public long getClientIdByName(String targetClientName) {
